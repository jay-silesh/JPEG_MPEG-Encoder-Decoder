    ...
    else if(case_no==3)
    {
   	  	...
   	  	for(int bit=24;bit<32;bit++)
    	{
    		double[][][] sub_blocks2_r_q=new double[divide_size][8][8];
           	...
           	dequantize(sub_blocks_r_DCT,sub_blocks2_r_q,quantization_number);
        	dequantize(sub_blocks_g_DCT,sub_blocks2_g_q,quantization_number);
        	dequantize(sub_blocks_b_DCT,sub_blocks2_b_q,quantization_number);
           	...
           	for(int q=0;q<divide_size;q++)
    		{
    			for(int y=0;y<8;y++)
    			{
    				for(int x=0;x<8;x++)
    				{
    					sub_blocks2_r[q][y][x]=((int)sub_blocks2_r_q[q][y][x]) & (Bit_Mapping[bit]);
    					sub_blocks2_g[q][y][x]=((int)sub_blocks2_g_q[q][y][x]) & (Bit_Mapping[bit]);
    					sub_blocks2_b[q][y][x]=((int)sub_blocks2_b_q[q][y][x]) & (Bit_Mapping[bit]);
          			}
        	...
        	calculate_idct(sub_blocks2_r,sub_blocks_r_iDCT);
            calculate_idct(sub_blocks2_g,sub_blocks_g_iDCT);
            calculate_idct(sub_blocks2_b,sub_blocks_b_iDCT);
...
static void merge_3darrays(double[][][] sub_blocks_r_DCT,double[][][] sub_blocks_g_DCT, double[][][] sub_blocks_b_DCT,double[][][] sub_blocks_DCT) {
	...
	for(q=0;q<divide_size;q++)
	{
		for(y=0;y<8;y++)
		{
			for(x=0;x<8;x++)
			{
				short r =   (short) sub_blocks_r_DCT[q][y][x];
				short g =   (short) sub_blocks_g_DCT[q][y][x];
				short b =   (short) sub_blocks_b_DCT[q][y][x]; 
				int pix = 0xff000000 | ((r & 0xff) << 16) | ((g & 0xff) << 8) | (b & 0xff);
	...
	{	 
		for (int x = 0; x <= 7; x++) 
        {
            for (int y = 0; y <= 7; y++) 
            {
                ...
                if(sum>255)
                	sum=255;//Math.round(127);
                else if(sum<0)
                	sum=0;//Math.round(-128);
                else
                	sum=Math.round(sum);
	...
	static void quantize(double[][][] sub_blocks_r_DCT,int n) {
		...
		for(q=0;q<divide_size;q++)
		{
			for(y=0;y<8;y++)
			{
				for(x=0;x<8;x++)
				{
					sub_blocks_r_DCT[q][y][x]=Math.round(((sub_blocks_r_DCT[q][y][x]/(Math.pow(2, n)))));
				}
			}
		}
